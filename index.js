...
setInterval(function() {
  ...
  client.user.setActivity(`${PREFIX}monitor | Watching ${count} website`);
}, 240000);
...
client.on("message", async message => {
  ...
  if (command == "monitor") {
    ...
    if (check) {
      if (check.link.length === LINKLIMTE) {
        return send(
          `You reached your limit, you can not add more than ${LINKLIMTE} website.`,
          message,
          "YELLOW"
        );
      }
...
.addField(`${PREFIX}monitor`, `Monitor Your Projects`)
...
.setFooter('Created By TOM YT #4545')
.setColor('RANDOM')
.setTimestamp()
